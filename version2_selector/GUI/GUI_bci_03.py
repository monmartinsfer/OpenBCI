...
class GUI():
    ...
    def __init__(self, app, callbacks):         #Recibe la aplicación completa y una lista de callbacks DESDE MAIN
        ...
        for i in range(len(self.bci_graph.channels_checkBox_list)):
            self.bci_graph.channels_checkBox_list[i].toggled.connect(lambda: self.set_ChannelDisplay())
    ...
    def eeg_update(self):
        ...
        if not self.selector:
            ...
            index = []
            for i in range(len(self.bci_graph.channels_checkBox_list)):
                if self.bci_graph.channels_checkBox_list[i].isChecked():
                    index.append(i)
            for j in range(len(self.curves_EEG_Selector)):
                self.curves_EEG_Selector[j].setData(sample[index[j], :])
    ...
    def initFilteringComboBox(self):
        self.bci_graph.filtering_comboBox.addItems(['Butter','EAWICA','AICAW'])
    ...
    def initChannelSelector(self):
        if self.app.constants.CHANNELS == 8:
            for i in range(8):
                self.bci_graph.channels_grid.itemAtPosition(i, 1).widget().setVisible(False)

        if self.app.constants.CHANNELS == 16:
            for i in range(8):
                self.bci_graph.channels_grid.itemAtPosition(i, 1).widget().setVisible(True)


    def set_ChannelDisplay(self):
        self.curves_EEG_Selector = []
        checked = 0
        ...
        for i in range(len(self.bci_graph.channels_checkBox_list)):
            if self.bci_graph.channels_checkBox_list[i].isChecked():
                ...
                c.setPos(0, 0)
                self.bci_graph.EEG_plot.addItem(c)  # Agregar la curva al widget
                self.curves_EEG_Selector.append(c)
                checked += 1
